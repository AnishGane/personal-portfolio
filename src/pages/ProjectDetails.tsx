...
const ProjectDetails = () => {
  ...
  return (
    <div className="mt-10">
      ...
      <div className="text-neutral-8 font-tooltip mt-14">
        <img
        ...
        <div className="mt-8 flex items-center gap-4">
          <span className="rounded-md bg-black px-1.5 py-[3px] text-sm font-medium text-white dark:bg-white dark:text-black">
        ...
        <h1 className="my-4 text-5xl font-medium">{projectContent.name}</h1>
        <p className="text-neutral-6 mt-2 leading-6.5 tracking-wide">
        ...
        <div className="tracking-wide">
          <div className="mb-8 text-[34px] leading-10 font-semibold">
            <AppendHashtag leftOffset={6}>
              <h2>{projectContent.subName}</h2>
            </AppendHashtag>
          </div>

          <div className="mb-8">
            <AppendHashtag NoOfHash={2} className="mb-5 text-3xl font-medium">
              <h3>Overview</h3>
            </AppendHashtag>
          ...
          <div className="mb-8">
            ...
            <ul className="mx-10 list-disc">
              {projectContent.itemData.Features.map((item, index) => (
                <li key={index} className="text-neutral-6 text-[15px] leading-8 font-light">
                  {item}
                </li>
              ))}
            </ul>
              ...
              {relatedProjects.length > 0 ? (
                relatedProjects.slice(0, 2).map((item) => (
                  ...
                  >
                    ...
                    <p className="text-neutral-6 line-clamp-2 text-xs leading-4.5">
                      {item.description}
                    </p>
...
const TechSpan = ({ item }: { item: string }) => {
  return <span className="ring-neutral-6/40 rounded-md px-1.5 py-0.5 text-xs ring-1">{item}</span>;
};
...
const AppendHashtag = ({
  ...
  return (
    <div className={`group relative flex w-fit items-start focus-within:outline-none ${className}`}>
      <span
        aria-hidden="true"
        className={`absolute -left-${leftOffset} font-bold text-neutral-400 opacity-0 transition-all duration-200 ease-in-out select-none group-focus-within:opacity-100 group-hover:opacity-100`}
      >
        {'#'.repeat(NoOfHash)}
      </span>
      {children}
